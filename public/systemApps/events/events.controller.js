...
angular.module("ApioApplication").controller("ApioEventsController", ["$scope", "$http", "socket", "$timeout", function ($scope, $http, socket, $timeout) {
    ...
    function _saveEvent() {
        ...
        $http.put("/apio/event/" + $scope.currentEvent.name, {eventUpdate: $scope.currentEvent}).success(function (data) {
    ...
    $scope.stateInList = function (item) {
        for (var i in $scope.currentEvent.triggeredStates) {
            if ($scope.currentEvent.triggerState === item.name || $scope.currentEvent.triggeredStates[i].name === item.name) {
                return false;
            }
        }
        return true;
     ...
     * socket.on("apio_event_delete", function (data) {
     for (var i in $scope.groupedEvents) {
     for (var j in $scope.groupedEvents[i]) {
     if (data.name === $scope.groupedEvents[i][j].name) {
     $scope.groupedEvents[i].splice(j, 1);
     }
     }

     if ($scope.groupedEvents[i].length === 0) {
     delete $scope.groupedEvents[i];
     }
     }
     });

     socket.on("apio_event_new", function (data) {
     $http.get("/apio/state").success(function (states) {
     for (var i in states) {
     if (data.triggerState === states[i].name) {
     if (typeof $scope.groupedEvents[states[i].objectName] === "undefined") {
     $scope.groupedEvents[states[i].objectName] = [];
     }
     $scope.groupedEvents[states[i].objectName].push(data);
     }
     }
     $scope.groupedEvents = order($scope.groupedEvents);
     }).error(function (error) {
     console.log("Error while getting state: ", error);
     });
     });

     socket.on("apio_event_update", function (data) {
     for (var i in $scope.groupedEvents) {
     for (var j in $scope.groupedEvents[i]) {
     if (data.event.name === $scope.groupedEvents[i][j].name) {
     $scope.groupedEvents[i][j].triggerState = data.event.triggerState;
     $scope.groupedEvents[i][j].triggerTimer = data.event.triggerTimer;
     $scope.groupedEvents[i][j].triggeredStates = data.event.triggeredStates;
     $scope.groupedEvents[i][j].type = data.event.type;
     }
     }
     }
     });*/
    ...
    socket.on("apio_event_delete", function (data) {
        for (var i in $scope.groupedEventsMap) {
            for (var j in $scope.groupedEventsMap[i]) {
                if (data.name === $scope.groupedEventsMap[i][j].name) {
                    $scope.groupedEventsMap[i].splice(j, 1);
                }
            }

            if ($scope.groupedEventsMap[i].length === 0) {
                delete $scope.groupedEventsMap[i];
                for (var j in $scope.groupedEventsList) {
                    if ($scope.groupedEventsList[j] === i) {
                        $scope.groupedEventsList.splice(j, 1);
                    }
                }
            }
        }
    ...
    socket.on("apio_event_new", function (data) {
        if (data.type === "timeTriggered") {
            if (typeof $scope.groupedEventsMap["Eventi temporali"] === "undefined") {
                $scope.groupedEventsList.push("Eventi temporali");
                $scope.groupedEventsMap["Eventi temporali"] = [];
            }
            $scope.groupedEventsMap["Eventi temporali"].push(data);
        } else if (data.type === "stateTriggered") {
            $http.get("/apio/state").success(function (states) {
                for (var i in states) {
                    if (data.triggerState === states[i].name) {
                        if ($scope.groupedEventsList.indexOf(states[i].objectName) === -1) {
                            $scope.groupedEventsList.push(states[i].objectName);
                        }

                        if (typeof $scope.groupedEventsMap[states[i].objectName] === "undefined") {
                            $scope.groupedEventsMap[states[i].objectName] = [];
                        }
                        $scope.groupedEventsMap[states[i].objectName].push(data);
                    }
                }
            }).error(function (error) {
                console.log("Error while getting state: ", error);
    ...
    socket.on("apio_event_update", function (data) {
        for (var i in $scope.groupedEventsMap) {
            for (var j in $scope.groupedEventsMap[i]) {
                if (data.event.name === $scope.groupedEventsMap[i][j].name) {
                    $scope.groupedEventsMap[i][j].triggerState = data.event.triggerState;
                    $scope.groupedEventsMap[i][j].triggerTimer = data.event.triggerTimer;
                    $scope.groupedEventsMap[i][j].triggeredStates = data.event.triggeredStates;
                    $scope.groupedEventsMap[i][j].type = data.event.type;
                }
            }
        }
    });
    ...
    $scope.loadEvents = function () {
        $http.get("/apio/event").success(function (data) {
            ...
            $scope.groupedEventsList = [];
            $scope.groupedEventsMap = {};

            for (var i in data) {
                if (data[i].type === "timeTriggered") {
                    $scope.groupedEventsList.push("Eventi temporali");
                    if (typeof $scope.groupedEventsMap["Eventi temporali"] === "undefined") {
                        $scope.groupedEventsMap["Eventi temporali"] = [];
                    }

                    $scope.groupedEventsMap["Eventi temporali"].push(data[i]);
                }
            }
            ...
            $http.get("/apio/state").success(function (states) {
                for (var i in data) {
                    for (var j in states) {
                        if (data[i].type === "stateTriggered" && data[i].triggerState === states[j].name) {
                            if ($scope.groupedEventsList.indexOf(states[j].objectName) === -1) {
                                $scope.groupedEventsList.push(states[j].objectName);
                            }

                            if (typeof $scope.groupedEventsMap[states[j].objectName] === "undefined") {
                                $scope.groupedEventsMap[states[j].objectName] = [];
                            }

                            $scope.groupedEventsMap[states[j].objectName].push(data[i]);
                        }
                    }
                }
                //$scope.groupedEvents = order($scope.groupedEvents);
                $scope.groupedEventsList.sort(function (a, b) {
                    if (a === "Eventi temporali") {
                        return -1;
                    } else if (b === "Eventi temporali") {
                        return 1;
                    } else {
                        return a.toLowerCase() > b.toLowerCase();
                    }
                });
                console.log("$scope.groupedEventsList: ", $scope.groupedEventsList);
                console.log("$scope.groupedEventsMap: ", $scope.groupedEventsMap);
            }).error(function (error) {
                console.log("Error while getting state with name " + data[i].triggerState + ", :", error);
            });
        }).error(function () {
    ...
    $scope.eventNameIsValid = function () {
        ...
        return typeof $scope.newEvent.name !== "undefined" && $scope.newEvent.name !== "";
    };
    ...
    $scope.saveEvent = function () {
        console.log("Mi accingo a salvare il seguente evento", $scope.newEvent);
        $http.post("/apio/event", {
            ...
            console.log("An error has occurred while saving the event, error: ", error);
        });
    ...
    $scope.deleteEvent = function (event) {
        $http.delete("/apio/event/" + event.name).success(function (data) {
    ...
    $scope.showForward = function () {
        ...
        switch ($scope.currentFormStep) {
            case "selezioneStatoScatenante":
                return $scope.newEvent.hasOwnProperty("triggerState");
            case "selezioneStatiScatenati":
                return $scope.newEvent.hasOwnProperty("triggeredStates") && haAlmenoUnoStatoScatenato();
            case "selezioneData":
                return $scope.newEvent.hasOwnProperty("triggerTimer");
        }
    };
    ...
    $scope.goToFormStep = function (step) {
        ...
        } else if (step == "selezioneStatoScatenante") {
    ...
    // FINE NUOVA PARTE SEZIONE LOGICHE
}]);
