...
angular.module("ApioDashboardApplication").controller("ApioDashboardSMSController", ["$scope", "$http", function ($scope, $http) {
        ...
        if ($scope.newNumberModel) {
            ...
            if (isIn) {
                ...
                $http.post("/apio/user/modifyAdditionalInfo", {
                    ...
                    $http.post("/apio/service/notification/route/" + encodeURIComponent("/apio/sms/addContact") + "/data/" + encodeURIComponent(JSON.stringify({
                            contact: $scope.newNumberModel,
                            email: $scope.loggedUser.email
                        }))).success(function () {
                        console.log("Data of service sms successfully modified");
                        $scope.newNumberModel = "";
                    }).error(function (error) {
                        console.log("Error while sending request to /apio/sms/addContact: ", error);
                    });
                }).error(function (err) {
                    $scope.newNumberModel = "";
            ...
            for (var j in $scope.service.data[$scope.objectId].properties[i]) {
                for (var k in $scope.service.data[$scope.objectId].properties[i][j].users) {
                    if ($scope.loggedUser.email === $scope.service.data[$scope.objectId].properties[i][j].users[k].email && $scope.currentNotification === $scope.service.data[$scope.objectId].properties[i][j].users[k].message) {
                        ...
                        $http.post("/apio/service/notification/route/" + encodeURIComponent("/apio/sms/newText") + "/data/" + encodeURIComponent(JSON.stringify({
                                email: $scope.loggedUser.email,
                            ...
                            console.log("Error while sending request to /apio/sms/newText: ", error);
                        });
                    }
                }
            }
    ...
    $scope.removeNumber = function (s) {
        ...
        $http.post("/apio/user/modifyAdditionalInfo", {
            ...
            $http.post("/apio/service/notification/route/" + encodeURIComponent("/apio/sms/removeContact") + "/data/" + encodeURIComponent(JSON.stringify({
                    contact: s.number,
                    email: $scope.loggedUser.email
                }))).success(function () {
                console.log("Data of service sms successfully modified");
            }).error(function (error) {
                console.log("Error while sending request to /apio/sms/removeContact: ", error);
            });
        }).error(function (err) {
            console.log("Error while updating addition_info, error: ", err);
        });
    ...
    $scope.toggleSend = function (dataNumber) {
        if ($scope.service && $scope.service.data) {
            ...
            $http.post("/apio/service/notification/route/" + encodeURIComponent("/apio/sms/toggleEnable") + "/data/" + encodeURIComponent(JSON.stringify({
                    email: $scope.loggedUser.email,
                    contact: dataNumber.contact,
                    notification: $scope.currentNotification,
                    objectId: $scope.objectId
                }))).success(function () {
                console.log("Data of service notification sms modified");
            }).error(function (error) {
                console.log("Error while sending request to /apio/sms/toggleEnable: ", error);
            });
        }
    };

    $scope.sms_ready = false;
    $http.get("/apio/user/getSessionComplete").success(function (session) {
        ...
        $http.post("/apio/user/getUser", {email: $scope.loggedUser.email}).success(function (data) {
            ...
            if (data.hasOwnProperty("user") && data.user.hasOwnProperty("additional_info") && data.user.additional_info.hasOwnProperty("sms")) {
                $scope.sms = data.user.additional_info.sms;
            ...
            $http.get("/apio/database/getObjects").success(function (objects) {
                $http.get("/apio/service/notification/route/" + encodeURIComponent("/apio/sms/getService?user=" + $scope.loggedUser.email)).success(function (service) {
