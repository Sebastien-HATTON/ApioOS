...
apioProperty.directive("battery", ["currentObject", "socket", "$http", function (currentObject, socket, $http) {
    return {
        restrict: "E",
        replace: true,
        scope: {
        ...
        templateUrl: "apioProperties/Battery/battery.html",
        link: function (scope, elem, attrs) {
            ...
            var volt = Number(scope.model);
            var percvalue = (max - min) / 100;
            //alert(scope.percvalue)
            var perc = (volt - min) / percvalue;
            scope.battPerc = parseInt(perc);
            if (scope.battPerc >= 31 && scope.battPerc <= 100) {
                document.getElementById("batt").style.height = scope.battPerc + "%";
                document.getElementById("batt").className = "battery-level";
            } else if (scope.battPerc >= 15 && scope.battPerc <= 30.999) {
                document.getElementById("batt").style.height = scope.battPerc + "%";
                document.getElementById("batt").className = "battery-level warn";
            } else if (scope.battPerc >= 0 && scope.battPerc <= 14.999) {
                document.getElementById("batt").style.height = scope.battPerc + "%";
                document.getElementById("batt").className = "battery-level alertt";
            }
                ...
                if (data.apioId === scope.object.apioId && data.objectId === scope.object.objectId && currentObject.isRecording() == false) {
                    if (data.properties.hasOwnProperty(scope.propertyname)) {
                        ...
                        if (attrs["push"]) {
                            ...
                            var volt = scope.model;
                            volt = Number(scope.model);
                            scope.percvalue = (max - min) / 100;
                            //alert(scope.percvalue)
                            var perc = (volt - min) / scope.percvalue;
                            //alert(perc);
                            scope.battPerc = parseInt(perc);
                            if (scope.battPerc >= 31 && scope.battPerc <= 100) {
                                document.getElementById("batt").style.height = scope.battPerc + "%";
