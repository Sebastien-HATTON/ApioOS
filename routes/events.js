                ...
                if (err){
                    ...
                    if(Apio.Configuration.type=="gateway"){
			            if(Apio.Configuration.remote.enabled==true){
				            Apio.Remote.socket.emit("apio.server.event.create", req.body.event);
			            }
			         } else {
			         	Apio.io.emit('apio.remote.event.create',req.body.event);
                ...
                if (err)
                	...
                	if(Apio.Configuration.type=="gateway"){
			            if(Apio.Configuration.remote.enabled==true){
			         	...
			         	Apio.io.emit('apio.remote.event.update',{name: req.params.name,eventUpdate:req.body.eventUpdate});
        ...
        delete: function(req, res) {
            ...
            Apio.Event.delete(req.params.name,function(err){
                if (err)
                	...
                	if(Apio.Configuration.type=="gateway"){
			            if(Apio.Configuration.remote.enabled==true){
				            Apio.Remote.socket.emit('apio.server.event.delete',{name: req.params.name})
			            }
			         } else {
			         	Apio.io.emit('apio.remote.event.delete',{name: req.params.name});
