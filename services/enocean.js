...
////var port = require("./services_setup.js").enocean.port;
//    var socket = require("socket.io-client")("http://localhost:" + configuration.http.port);
////var socketDongle = require("socket.io-client")("http://localhost:" + require("./services_setup.js").dongle.port);
...
//socket.listen(server);
    ...
    var configuration = require("../configuration/default.js");
    var enocean = libraries["node-enocean"]();
    ...
    //var socket = libraries["socket.io-client"]("http://localhost:" + configuration.http.port, {query: "apioId=" + Apio.System.getApioIdentifier()});
    //var socket = libraries["socket.io-client"]("http://localhost:" + configuration.http.port, {query: "associate=enocean"});
    var socket = libraries["socket.io-client"]("http://localhost:" + configuration.http.port, {query: "associate=enocean&token=" + Apio.Token.getFromText("enocean", fs.readFileSync("./" + Apio.Configuration.type + "_key.apio", "utf8"))});
...
//Write your code
            ...
            if (data.learnBit != '0' && data.choice != 'd4') {
                ...
                if (data.packetTypeString == '4BS') {
                    ...
                    if (data.hasOwnProperty('eep')) {
                        ...
                        //enoceanSend('','01','02')
                        //SPOSTATO PER PROVA, PRIMA ERA PARALLELO A QUESTO IF IN CUI E CONTENUTO
                ...
                if (!bindToProperty.enocean.hasOwnProperty(data.senderId)) {
                    ...
                    if (communication.enocean.hasOwnProperty(eepCalculated) && communication.enocean[eepCalculated].send.hasOwnProperty('setup')) {
                        console.log();
                        console.log('il dispositivo prevede una funzione di setup...');
                        var dataObject = {
                            protocol: {
    ...
    var enoceanSend = function (enROR, enPacketType, enData, enDestinationId, enLearn) {
        if (enROR == 'd2' || enROR == 'D2') {
            if (enPacketType == '01') {
                ...
                //di seguito l'id del dongle enocean prelevato con il comando 05, poi si dovrà studiare come aggiornare l'enocean base commentsato di sopra. Attualmete quindi se vuoi fare le prove devi utilizzare il dongle enocea che è collegato sulla 0.4 che ha il chipId:
                //019e66d0
                //ffb90900
                //ffb90900
                var senderId = enocean.base;
        ...
        if (err) {
            ...
            ports.forEach(function (port) {
                ...
                if (String(port.manufacturer) === "EnOcean_GmbH" || String(port.manufacturer) === "EnOcean_GmbH") {
                    ...
                    //Leggo l'enocean.base e lo confronto con ciò che leggo dal comando e poi scrivo sul dongle l'enocean base se sono diversi
                    console.log()
                    /*setTimeout(function(){
                     enoceanSend('00','05', '08','ffffffff');
                     }, 20000)*/
                    enocean.on("data", function (data) {
                        console.log("Roba che arriva ", data)
                        enoceanApplianceOnOnce = 1;
    ...
    socketServer.on("connection", function (Socket) {
        Socket.on("update_collections", function (data) {
            console.log("###############update_collections: ", data);
    ...
    http.listen(Port, function () {
        console.log("APIO Enocean Service on " + Port);
    });
};