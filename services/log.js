...
socket_server.on("connection", function (socket) {
    ...
    socket.on("log_require", function (data) {
        ...
        sql_db.query("SELECT * FROM `" + data.objectId + "` ORDER BY timestamp DESC LIMIT " + usersLogsFiles[data.user][data.objectId] + ", 50", function (error, result) {
            if (error) {
    ...
    database.collection("Objects").findOne({objectId: req.params.objectId}, function (err, object) {
        if (err) {
            ...
            database.collection("Services").findOne({name: "log"}, function (err1, service) {
                ...
                sql_db.query("SELECT * FROM `" + req.params.objectId + "` ORDER BY timestamp DESC LIMIT 0, " + (limit * limit), function (error, result) {
                    if (error) {
                        console.log("Error while getting logs from table " + req.params.objectId + ": ", error);
                    } else {
                        var obj = {};
...
app.get("/apio/log/exportXLSX", function (req, res) {
    ...
    database.collection("Objects").findOne({objectId: req.query.objectId}, function (err, object) {
        if (err) {
            ...
            sql_db.query("SELECT * FROM `" + req.params.objectId + "` ORDER BY timestamp DESC LIMIT 0, " + (50 * 250), function (error, result) {
                if (error) {
                    console.log("Error while getting logs from table " + req.params.objectId + ": ", error);
                } else {
                    var obj = {};
...
app.get("/apio/log/getByDate/objectId/:objectId/date/:date", function (req, res) {
    ...
    req.pause();
    sql_db.query("SELECT * FROM `" + req.params.objectId + "` WHERE (timestamp >= '" + todayTimestamp + "' AND timestamp < '" + tomorrowTimestamp + "') ORDER BY timestamp DESC").on("error", function () {
        res.status(200).send({});
...
app.get("/apio/log/getByRange/objectId/:objectId/from/:from/daysNumber/:daysNumber", function (req, res) {
    ...
    req.pause();
    sql_db.query("SELECT * FROM `" + req.params.objectId + "` WHERE (timestamp >= '" + startTimestamp + "' AND timestamp < '" + endTimestamp + "') ORDER BY timestamp DESC").on("error", function () {
        res.status(200).send({});
    }).on("result", function (row) {
        processRow(row);
    }).on("end", function () {
