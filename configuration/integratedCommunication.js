module.exports = {
    ...
    "apio": {
        ...
        "SenecaMonS600": {
            "send": {},
            "recive": {
                "energy": "(function(data) { console.log('*******data******', data); var properties = { dati: { 'frequency': '', 'voltage': ' ', 'current': '', 'power': '', 'energy': '', 'impEnergy': '', 'expEnergy': '', 'impLagAppEnergy': '', 'expLagAppEnergy': '' } }; var property = Object.keys(data.properties)[0]; if (property == 'SenecaMonS600') { var value = data.properties.SenecaMonS600.split('|'); var n = Number(value[0]); for (var h = 1; h < value.length - 1; h++) { if(value[h][0] == 'n'){ console.log('valore negativo'); value[h] = value[h].substring(0, 0) + '-' + value[h].substring(0+1); } properties.dati[Object.keys(properties.dati)[n]] = value[h]; console.log('n: ', n); console.log('h: ', h); console.log('this property: ', properties.dati[Object.keys(properties.dati)[n]]); n++; } console.log('******properties: ', properties.dati); } var key = Object.keys(properties.dati); for(var x in key){ if(properties.dati[key[x]] == ''){ delete properties.dati[key[x]]; } } return properties.dati; })",
                "update": "(function(data){console.log('switch');var properties = {'switch':''}; properties.switch=String(Number(data.value)); return properties;})"
            }
        },
        "SenecaTriS600": {
            "send": {},
            "recive": {
                "energy": "(function(data) { console.log('*******data******', data); var properties = { dati: { 'frequency': '', 'voltage1': '', 'voltage2': '', 'voltage3': '', 'voltage': '', 'current1': '', 'current2': '', 'current3': '', 'current': '', 'power1': '', 'power2': '', 'power3': '', 'power': '', 'apparentPower1': '', 'apparentPower2': '', 'apparentPower3': '', 'apparentPower': '', 'reactivePower1': '', 'reactivePower2': '', 'reactivePower3': '', 'reactivePower': '', 'impEnergy': '', 'expEnergy': '', 'impLagAppEnergy': '', 'expLagAppEnergy': '' } }; var property = Object.keys(data.properties)[0]; if (property == 'SenecaTriS600') { var value = data.properties.SenecaTriS600.split('|'); var n = Number(value[0]); for (var h = 1; h < value.length - 1; h++) { console.log('primo valore: ',value[h][0]); if(value[h][0] == 'n'){ console.log('valore negativo'); value[h] = value[h].substring(0, 0) + '-' + value[h].substring(0+1); } properties.dati[Object.keys(properties.dati)[n]] = value[h]; console.log('n: ', n); console.log('h: ', h); console.log('this property: ', properties.dati[Object.keys(properties.dati)[n]]); n++; } console.log('******properties: ', properties.dati); } var key = Object.keys(properties.dati); for(var x in key){ if(properties.dati[key[x]] == ''){ delete properties.dati[key[x]]; } } return properties.dati; })"
            }
        },
        "DIN8": {
            "send": {
